...
func WorkerLogin(session sessions.Session, db *sql.DB, r render.Render, req *http.Request) {
...
func WorkerLogout(session sessions.Session, r render.Render) {
	session.Delete("admin_userid")
	r.HTML(200, "login", "登出成功")
}

func WorkerAuth(session sessions.Session, c martini.Context, r render.Render) {
