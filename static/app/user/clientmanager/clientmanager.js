...
app.controller('ClientManagerCtrl', function ($scope, i18nService, $modal, $log, cfpLoadingBar, $http, $window) {
    ...
    $scope.clients = [];
    $http.get("/user/clients").then(function (ret) {
    ...
    $scope.client_add = function (name) {
        $http.get("/user/client/add/" + name).then(function (ret) {
    ...
    $scope.client_del = function (c, idx) {
        if ($window.confirm("确定删除设备\"" + c._id + "\"吗?") == 1) {
            $http.get("/user/client/del/" + c._id).then(function (ret) {
    ...
    $scope.video_save = function (c, v,idx) {
        ...
        if (v.new_id && v.new_id != v._id) {
            if (!v._id) {
                $http.get("/user/client/videoadd/" + c._id + '/' + v.new_id).then(function (ret) {
                    v._id = v.new_id;
                    v.show = false;
                });
            } else {
                $http.get("/user/client/videochange/" + c._id + '/' + idx + '/' + v.new_id).then(function (ret) {
                    v._id = v.new_id;
                    v.show = false;
                });
            }
